...
public class PaintingLocationsFile extends SimpleFile<FakeAPI> {
    public PaintingLocationsFile(FakeAPI plugin) {
        super("plugins/FakeAPI", "fake_painting.ht", plugin);
    }
    ...
    public HashMap<String, Location> getPacketPainting() {
        ...
        for (String names : getKeys(false)) {
            paintingmap.put(names.replace("_", " ").replace("&", "§"), getLocation(names));
        }
        return paintingmap;
    }
}
