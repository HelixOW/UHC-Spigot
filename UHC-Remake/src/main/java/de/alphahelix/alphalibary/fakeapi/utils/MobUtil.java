...
public class MobUtil extends UtilBase {
     ...
     */
    public static FakeMob spawnMob(Player p, Location loc, String name, FakeMobType type) {
        try {
     ...
     * @return the new spawned {@link FakeMob}
     */
    public static FakeMob spawnTemporaryMob(Player p, Location loc, String name, FakeMobType type) {
        try {
            Object mob = ReflectionUtil.getNmsClass(type.getNmsClass()).getConstructor(ReflectionUtil.getNmsClass("World")).newInstance(ReflectionUtil.getWorldServer(p.getWorld()));
     ...
     */
    public static void equipMob(Player p, FakeMob mob, ItemStack item, EquipSlot slot) {
        try {
    ...
    public static void followPlayer(final Player p, final Player toFollow, final FakeMob mob) {
        followMap.put(p.getName(), new BukkitRunnable() {
            @Override
            public void run() {
                teleportMob(p, FakeAPI.getLocationBehindPlayer(toFollow, 2), mob);
            }
    ...
    public static void equipMobSkull(Player p, FakeMob mob, String textureURL) {
        try {
            equipMob(p, mob, SkullItemBuilder.getSkull(textureURL), EquipSlot.HELMET);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static void equipMobSkull(Player p, FakeMob mob, GameProfile profile) {
        try {
            equipMob(p, mob, SkullItemBuilder.getSkull(profile), EquipSlot.HELMET);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
