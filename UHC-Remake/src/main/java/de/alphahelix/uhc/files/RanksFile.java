 ...
 */
public class RanksFile extends SimpleFile<UHC> {
    ...
    public void initRank(Player p) {
        long kills = UHCRegister.getStatsUtil().getKills(p);
        long wins = UHCRegister.getStatsUtil().getWins(p);
        long points = UHCRegister.getStatsUtil().getPoints(p);
        ...
        UHCRegister.getStatsUtil().setUhcRank(toPut, p);
    }
}
