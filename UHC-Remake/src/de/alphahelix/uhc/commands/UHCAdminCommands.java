	...
	public boolean execute(CommandSender cs, String label, String[] args) {
		...
		if (args.length == 0) {
			TextComponent msg = new TextComponent(
					getPlugin().getPrefix() + "§7Please click §chere §7to see the wiki");
			...
			if (args[0].equalsIgnoreCase("push")) {
				getPlugin().getRegister().getStatsUtil().pushCacheToBackUp();
			...
			if (args[0].equalsIgnoreCase("createKit")) {
				...
				p.sendMessage(
						getPlugin().getPrefix() + "§7You have set the kit §a" + args[1] + " §7with GUI-block §a"
