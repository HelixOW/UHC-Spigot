	...
	public static Scenarios[] getScenariosToChoose() {
		...
		for(int i = 0; i < Scenarios.values().length; i++) {
			if(tr.size() < 4) {
				...
				if(UHC.getInstance().getRegister().getScenarioFile().isEnabled(Scenarios.getRawScenarioName(tA)))
					if(!tr.contains(tA))
						tr.add(tA);
			}
		}
