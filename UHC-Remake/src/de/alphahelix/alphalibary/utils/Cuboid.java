            ...
            for (int z = z1; z <= z2; z += 16) {
                ...
                if (current % 5000 == 0) {
                    ...
                    Bukkit.getLogger().log(Level.INFO,
                            "Generating chunks... (" + current + "/" + max + ") ["
                                    + Math.round(current * 100.0F / (max / 1.0F) * 10.0F) / 10.0F + "%]" + " (RAM: "
                                    + Math.round((float) (Runtime.getRuntime().maxMemory()
                                    - Runtime.getRuntime().freeMemory()) * 100.0F
                                    / ((float) Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F) / 10.0F
                                    + "%)");
                }
                current++;
                if (Math.round((float) (Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory()) * 100.0F
                        / ((float) Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F) / 10.0F > 80.0F) {
                    while (Math
                            .round((float) (Runtime.getRuntime().maxMemory() - Runtime.getRuntime().freeMemory())
                                    * 100.0F / ((float) Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F)
                            / 10.0F > 60.0F) {
                        try {
                            Bukkit.getLogger().log(Level.INFO, "Memory usage too high, waiting... ["
                                    + Math.round(current * 100.0F / (max / 1.0F) * 10.0F) / 10.0F + "%]" + " (RAM: "
                                    + Math.round((float) (Runtime.getRuntime().maxMemory()
                                    - Runtime.getRuntime().freeMemory()) * 100.0F
                                    / ((float) Runtime.getRuntime().maxMemory() / 1.0F) * 10.0F) / 10.0F
