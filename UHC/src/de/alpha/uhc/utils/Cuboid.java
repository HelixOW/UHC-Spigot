...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
     ...
     */
    private Cuboid expand(CuboidDirection dir, int amount) {
        switch (dir) {
            case North:
     ...
     */
    private Cuboid shift(CuboidDirection dir) {
        return expand(dir, 1).expand(dir.opposite(), -1);
    }
     ...
     */
    private Cuboid outset(CuboidDirection dir, int amount) {
        Cuboid c;
        switch (dir) {
            case Horizontal:
     ...
     */
    private Cuboid contract(CuboidDirection dir) {
        Cuboid face = getFace(dir.opposite());
        switch (dir) {
            case Down:
                while (face.containsOnly() && face.getLowerY() > this.getLowerY()) {
                    face = face.shift(CuboidDirection.Down);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, this.x2, face.getUpperY(), this.z2);
            case Up:
                while (face.containsOnly() && face.getUpperY() < this.getUpperY()) {
                    face = face.shift(CuboidDirection.Up);
                }
                return new Cuboid(this.worldName, this.x1, face.getLowerY(), this.z1, this.x2, this.y2, this.z2);
            case North:
                while (face.containsOnly() && face.getLowerX() > this.getLowerX()) {
                    face = face.shift(CuboidDirection.North);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, face.getUpperX(), this.y2, this.z2);
            case South:
                while (face.containsOnly() && face.getUpperX() < this.getUpperX()) {
                    face = face.shift(CuboidDirection.South);
                }
                return new Cuboid(this.worldName, face.getLowerX(), this.y1, this.z1, this.x2, this.y2, this.z2);
            case East:
                while (face.containsOnly() && face.getLowerZ() > this.getLowerZ()) {
                    face = face.shift(CuboidDirection.East);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, face.getUpperZ());
            case West:
                while (face.containsOnly() && face.getUpperZ() < this.getUpperZ()) {
                    face = face.shift(CuboidDirection.West);
                }
     ...
     */
    private Cuboid getFace(CuboidDirection dir) {
        switch (dir) {
            case Down:
